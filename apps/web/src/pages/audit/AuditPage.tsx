    ...
    <div className="p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        <select
          ...
          className="bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-200 focus:border-brand-500 focus:outline-none"
        >
          ...
          <table className="w-full text-xs text-left">
            <thead className="text-[11px] uppercase tracking-wider text-slate-400 bg-slate-950/60 border-b border-slate-800">
              <tr>
                ...
                <th className="py-3 px-4">Changes & Values</th>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {loading && (!Array.isArray(logs) || logs.length === 0) ? (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-slate-500">Loading audit history...</td>
                </tr>
              ) : !Array.isArray(logs) || logs.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-slate-500">No audit events recorded.</td>
                </tr>
              ) : (
                logs.map(log => (
                  <tr key={log.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-3 px-4 font-semibold text-slate-300">
                      {log.entityType} <span className="text-[10px] text-slate-500 font-mono">({log.entityId.slice(0, 8)}...)</span>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4 text-[11px] font-mono text-slate-400 max-w-xs truncate">
